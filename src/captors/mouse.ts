...
class CaptorsMouse {
    ...
    public MouseMoveListener(e: MouseEvent) {
        ...
        if (!this.selected) {
            ...
            !this.draggable
            // @ts-ignore
                ? (this.hoverTatget = this[nodeFunckName](mouseX, mouseY))
            ...
            if (isNode && draggable && this.draggable) {
                ...
                if (hasRealSelect) {
                    ...
                    // 没按住ctrl则更新当前选中的点
                    this.geoEnable &&
                        this.hoverTatget.changeAttribute({
                            x: realX + x,
                            y: realY + y,
                        })
                    dragNodes = [id]
    ...
    private checkNode = (x: number, y: number) => {
        let sel = null
        // 得到包围盒
        let boundBox = basicData[this.scene.id].boundBox
        ...
        for (let [key, val] of boundBox) {
            ...
            if (basicData[this.scene.id].nodeList.get(key)?.getAttribute('opacity') == 0.0) continue
    ...
    private checkQuad = (x: number, y: number, type: boolean = true) => {
        let camera = this.camera
        // 检测那块区域的四叉树
        var items = camera.quad.retrieve({ x, y, height: 5, width: 5 })
        // item去重
        let map = new Map()
        for (let item of items) {
            if (!map.has(item.id) && item.isNode && type) {
                map.set(item.id, item)
            } else if (!map.has(item.id) && !item.isNode) {
                map.set(item.id, item)
            }
        }
        return map
    ...
    private checkNodeCanvas = (x: number, y: number) => {
        let width = 5,
            height = 5,
            map = this.checkQuad(x, y),
            items = [...map.values()]
        // 获取node的绘制顺序
        let noderOrder = [...globalInfo[this.scene.id].nodeOrder]
        ...
        for (let item of items) {
            ...
            // 碰撞大于0说明碰撞的盒子碰到了
            if (area > 0) {
                if (basicData[this.scene.id].nodeList.get(item.id)?.getAttribute('opacity') == 0.0)
                    continue
                // 获取当前点的顺序
        ...
        if (sel.id) {
            let id = sel.id
            let nodeOrder = globalInfo[this.scene.id].nodeOrder
            if (nodeOrder.has(id)) {
                nodeOrder.delete(id)
                nodeOrder.add(id)
            }
    ...
    private checkEdgeCanvas = (x: number, y: number) => {
        let width = 5,
            height = 5,
            map = this.checkQuad(x, y, false),
            items = [...map.values()]
        let edgeSel = { area: -1, index: -1, id: null }
        let edgeOrder = [...globalInfo[this.scene.id].edgeOrder]
        ...
        for (let item of items) {
            ...
            // 碰撞面积大于0等于盒子碰撞了
            if (area > 0) {
                if (basicData[this.scene.id].edgeList.get(item.id)?.getAttribute('opacity') == 0.0)
                    continue
        ...
        if (edgeSel.id != null) {
            ...
            let edgeOrder = globalInfo[this.scene.id].edgeOrder
            if (edgeOrder.has(id)) {
                let edgeList = basicData[this.scene.id].edgeList
        ...
        switch (Number(typeShape[shape])) {
            ...
            case 3:
                ...
                let x1 = x + radius * Math.sin(rotate),
                    ...
                    y2 = y - Math.cos(rotate + (2 * Math.PI * 1) / 3) * radius,
                    x3 = x + Math.sin(rotate + (2 * Math.PI * 2) / 3) * radius,
                    y3 = y - Math.cos(rotate + (2 * Math.PI * 2) / 3) * radius
    ...
    private shapeEdgeChek = (pointX: number, pointY: number, item: any) => {
        ...
        let edgeBoundBox = this.scene?.thumbnail
            ? basicData[this.scene.id].thumbnailEdgeBoundBox
            : basicData[this.scene.id].edgeCanvasBoundBox

        if (edgeBoundBox.has(id)) {
            let item = edgeBoundBox.get(id)
            let points = item.point
            for (let i = 0; i < points.length; i += 2) {
                if (
                    isInside(
                        points[i],
