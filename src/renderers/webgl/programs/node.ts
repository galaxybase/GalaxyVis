...
export default class NodeProgram extends AbstractNodeProgram {
    ...
    process(): void {
        ...
        const transform = basicData[id].transform
        const nodeList: Map<any, any> = basicData[this.graph.id].nodeList
        const needFresh = basicData[this.graph.id].selectedTable
        // 获取icon的列表
        ...
        for (let [key, val] of float32Nodes) {
            ...
            const isBadges = attributes.badges ? true : false
            let p: any = getPoint(this.graph.id, attributes, iconMap, transform)
            // 生成一个AABB的包围盒
            boundBox.set(key, {
        ...
        if (!collection.packedData.length) {
            nodeCollection[this.graph.id] = collection
            return
        }

        // 更新数据
        !this.graph.thumbnail && (basicData[this.graph.id].drawNodeList = drawNodeList)
        !this.graph.thumbnail && (basicData[this.graph.id].boundBox = boundBox)
