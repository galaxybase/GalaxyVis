 ...
 * @returns
 */
export default function canvasNodeSquare(
    graphId: string,
    ...
    // borderSize = borderSize ? borderSize / 100 : 2 / 100
    borderSize = Number(borderSize) >= 0 ? borderSize / 100 : 2 / 100
    // 根据相机位置更改点的初始位置
    size *= scale * Math.sqrt(2)
...
const squareGraphic = (context: CanvasRenderingContext2D, x: number, y: number, radius: number) => {
...
const drawBorder = (
    ...
    if (outLine) {
        ...
        squareGraphic(context, x, y, radius - outLine)
        context.closePath()
        context.stroke()
    }
    if (line_width != 0) {
        ...
        squareGraphic(context, x, y, radius - outLine / 4)
        context.closePath()
        context.stroke()
    }
}
