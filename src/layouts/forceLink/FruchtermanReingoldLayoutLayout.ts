...
class FruchtermanReingoldLayout extends BaseLayout {
    ...
    */
    ...
    execute(nodes: any[], edges: any[]) {
        try {
            this.data = fruchtermanReingoldLayout(nodes, edges, this.options)
            this.ids = []
            this.positions = []
            ...
            if (this.options.useWebWorker != false && typeof Worker !== 'undefined') {
                ...
                worker.onmessage = function (event: any) {
                    if (event.data.type == LAYOUT_MESSAGE.END) {
                        ...
                        animation(that, event, layoutsNodes, 'FruchtermanReingold').then((data) => {
                            worker.terminate()
                            resolve(data)
                        })
                    } else {
                ...
                try {
                    ...
                    animation(this, null, ids, 'FruchtermanReingold').then((data) => {
                        resolve(data)
                    })
                } catch (err) {
                    reject(LAYOUT_MESSAGE.ERROR)
