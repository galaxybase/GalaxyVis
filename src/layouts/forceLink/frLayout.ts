...
class frLayout extends BaseLayout {
    nodeList: any;
    nodeTable: any;
    edgeList: any;
    ...
    execute(nodes: any[], edges: any[]) {
        try {
            this.data = FRLayout(nodes, edges, this.options)
            this.ids = []
            this.positions = []
            ...
            if (this.options.useWebWorker != false && typeof Worker !== 'undefined') {
                ...
                worker.onmessage = function (event: any) {
                    if (event.data.type == LAYOUT_MESSAGE.END) {
                        ...
                        animation(that, event, layoutsNodes, 'fr').then((data) => {
                            worker.terminate()
                            cleartNodeList()
                            resolve(data)
                        })
                ...
                try {
                    ...
                    animation(this, null, ids, 'fr').then((data) => {
                        cleartNodeList()
                        resolve(data)
                    })
                } catch (err) {
