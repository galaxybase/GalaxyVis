...
import kkFaLayout from "./kk";

class kkLayout extends BaseLayout {
    nodeList: any;
    nodeTable: any;
    edgeList: any;
    ...
    execute(nodes: any[], edges: any[]) {
        try {
            let KKFa = new kkFaLayout(nodes, edges, this.options);
            KKFa.resetConfig(KKFa.getConfig());
            this.data = KKFa.runLayout();
            this.ids = []
            this.positions = []
            ...
            if (this.options.useWebWorker != false && typeof Worker !== 'undefined') {
                ...
                worker.onmessage = function (event: any) {
                    if (event.data.type == LAYOUT_MESSAGE.END) {
                        ...
                        animation(that, event, layoutsNodes, 'kk').then((data) => {
                            worker.terminate()
                            cleartNodeList()
                            resolve(data)
                        })
                ...
                try {
                    ...
                    animation(this, null, ids, 'kk').then((data) => {
                        cleartNodeList()
                        resolve(data)
                    })
                } catch (err) {
