...
class SphereLayout extends BaseLayout {
    ...
    execute(nodes: any[]) {
        try {
            this.data = sphereLayout(nodes, this.options)
            this.ids = []
            this.positions = []
            ...
            if (this.options.useWebWorker != false && typeof Worker !== 'undefined') {
                ...
                worker.onmessage = function (event: any) {
                    if (event.data.type == LAYOUT_MESSAGE.END) {
                        ...
                        animation(that, event, ids, 'sphere').then((data) => {
                            worker.terminate()
                            resolve(data)
                        })
                    } else {
                ...
                try {
                    ...
                    animation(this, null, ids, 'sphere').then((data) => {
                        resolve(data)
                    })
                } catch (err) {
                    reject(LAYOUT_MESSAGE.ERROR)
