...
import HierarchicalLayout from "./hubsize";
import { EventType } from "../../utils/events";

class HubSizeLayout extends BaseLayout {
    nodeList: any;
    nodeTable: any;
    edgeList: any;
     ...
     */
     execute(nodes: any[], edges: any[]) {
        try {
            let hubsize = new HierarchicalLayout(nodes, edges);
            ...
            if (this.options.useWebWorker != false && typeof Worker !== 'undefined') {
                ...
                worker.onmessage = function (event: any) {
                    if (event.data.type == LAYOUT_MESSAGE.END) {
                        ...
                        animation(that, event, layoutsNodes, 'hubsize').then((data) => {
                            worker.terminate()
                            cleartNodeList()
                            resolve(data)
                        })
                ...
                try {
                    ...
                    animation(this, null, ids, 'hubsize').then((data) => {
                        cleartNodeList()
                        resolve(data)
                    })
                } catch (err) {
