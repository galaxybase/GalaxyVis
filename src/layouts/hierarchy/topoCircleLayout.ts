...
import topoLayout from "./topo";
import { EventType } from "../../utils/events";

class topoCircleLayout extends BaseLayout {
    nodeList: any;
    nodeTable: any;
    edgeList: any;
    ...
    execute(nodes: any[]) {
        try {
            this.data = topoLayout(nodes, this.options);
            this.ids = []
            this.positions = []
    ...
    layout() {
        return new Promise(async (resolve, reject) => {
            ...
            if (this.options.useWebWorker != false && typeof Worker !== 'undefined') {
                ...
                worker.onmessage = function (event: any) {
                    if (event.data.type == LAYOUT_MESSAGE.END) {
                        ...
                        animation(that, event, layoutsNodes, 'topoCircle').then((data) => {
                            worker.terminate()
                            cleartNodeList()
                            resolve(data)
                        })
            ...
            try {
                ...
                animation(this, null, ids, 'topoCircle').then((data) => {
                    cleartNodeList()
                    resolve(data)
                })
            } catch (err) {
...
export default topoCircleLayout