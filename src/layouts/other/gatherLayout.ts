...
class GatherLayout extends BaseLayout {
    nodeList: any;
    nodeTable: any;
    edgeList: any;
    ...
    */
    ...
    execute(nodes: any[], edges: any[]) {
        try {
            this.data = gatherTypeLayout(nodes, edges, this.options)
            this.ids = []
            this.positions = []
            ...
            if (this.options.useWebWorker != false && typeof Worker !== 'undefined') {
                ...
                worker.onmessage = function (event: any) {
                    if (event.data.type == LAYOUT_MESSAGE.END) {
                        ...
                        animation(that, event, layoutsNodes, 'gather').then((data) => {
                            worker.terminate()
                            resolve(data)
                        })
                    } else {
                ...
                try {
                    ...
                    animation(this, null, ids, 'gather').then((data) => {
                        resolve(data)
                    })
                } catch (err) {
                    reject(LAYOUT_MESSAGE.ERROR)
